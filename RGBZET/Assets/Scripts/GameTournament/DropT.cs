...
public class DropT : MonoBehaviourPunCallbacks, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private MutiManageT mutiT;


    public void Start()
    {
        deck = FindObjectOfType<DeckT>(); // หรือใช้วิธีการค้นหาที่สอดคล้องกับโครงสร้างของโปรเจคของคุณ
        currentScore = 0; // เพิ่มบรรทัดนี้เพื่อกำหนดค่าเริ่มต้นของ currentScore
        mutiT = FindAnyObjectByType<MutiManageT>();
    }

    public void Update()
    {
        // เมื่อปุ่ม ZET ไม่ได้ถูกกด
        if (!MutiManageT.isZETActive)
        {
            ...
            if (numberOfCardsInCheckZone > 0 && numberOfCardsInCheckZone < 3)
            {
                ...
                for (int i = numberOfCardsInCheckZone - 1; i >= 0; i--) // เริ่มจากตัวสุดท้าย
                {
                    ...
                    if (cardTransform != null)
                    {
                        ...
                        DisplayCardT displayCardComponent = cardTransform.GetComponent<DisplayCardT>();
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (eventData.pointerDrag != null)
        {
            ...
            DragT draggable = eventData.pointerDrag.GetComponent<DragT>();
            if (draggable != null)
            {
            ...
            DisplayCardT displayCardComponent = eventData.pointerDrag.GetComponent<DisplayCardT>();
            if (displayCardComponent != null)
            {
    ...
    public void UpdateScore(int addScore)
    {
        ...
        mutiT.photonView.RPC("UpdatePlayerScore", RpcTarget.All, actorNumber, currentScore);
    ...
    public void RemoveCardsFromGameRPC()
    {
        ...
        for (int i = numberOfCards - 1; i >= 0; i--)
        {
            ...
            if (cardTransform != null)
            {
                DisplayCardT displayCard = cardTransform.GetComponent<DisplayCardT>();

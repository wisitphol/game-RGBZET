...
public class DragT : MonoBehaviourPunCallbacks, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    public Transform parentToReturnTo = null;
    private Vector3 startPosition;
    private Quaternion startRotation;
    private DisplayCardT displayCard;
    [SerializeField] private Outline outline;
    [SerializeField] private AudioSource audioSource;

    void Start()
    {
        displayCard = GetComponent<DisplayCardT>();
    ...
    {
        if (MutiManageT.isZETActive && MutiManageT.playerWhoActivatedZET == PhotonNetwork.LocalPlayer) // ตรวจสอบผู้เล่นที่กดปุ่ม ZET
        {
